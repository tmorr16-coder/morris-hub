...
export async function saveStudentSettings({
  ...
  try {
      ...
      .upsert(
        {
          ...
          lsat_enabled: lsatEnabled ?? false,
          lsat_target_score: lsatTargetScore ?? null,
    ...
    if (error) {
      console.error("saveStudentSettings error:", error);
      return { error: "Failed to save settings" };
    }
    return {};
  } catch (err) {
    console.error("saveStudentSettings unexpected error:", err);
