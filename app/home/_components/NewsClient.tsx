  ...
  return (
    <div>
      {topics.map((topic) => {
        ...
        const it = items[idx];
        if (!it) return null;
        const color = TOPIC_COLORS[topic] ?? "var(--ios-label-2)";
        return (
          <div key={topic} className="ios-cell" style={{ alignItems: "flex-start" }}>
            ...
            {items.length > 1 && (
              <span className="ios-cell-trail" style={{ flexDirection: "column", gap: 4 }}>
                <button onClick={() => prev(topic)} style={navBtn} aria-label="Previous story">
                  <Icons.ChevronLeft width={15} height={15} />
                </button>
                <button onClick={() => next(topic)} style={navBtn} aria-label="Next story">
