...
import { runMonteCarlo, MC_SIMS, MC_STDEV, MC_INFLATION_STDEV } from "../_lib/montecarlo";
...
export default function ProjectionTab({ profile, accounts, incomes, scenario, expenses, debts }: Props) {
  ...
  // The simulation itself lives in _lib/montecarlo so the plan document reads
  // the same numbers this chart shows.
  const mc = useMemo(() => {
    const ctx = buildCtx(profile, accounts, incomes, expenses, debts, scenario);
    return runMonteCarlo(ctx, ages);
